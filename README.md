...
1. **JIS配列キーボードのUS配列化 (ULE4JIS エミュレーション)**
   ...
   - `Shift` との組み合わせ記号（`@` `&` `*` `(` `)` `+` `:` `"` など）もUS配列通りにシミュレートします。

2. **キーボードデバイス自動識別 (1文字目学習・2文字目切り替え)** 🚀 *NEW*
   - Win32 Raw Input API を使用し、入力された最初の1文字目で「ノートPC本体のJISキーボード」か「外付けUSキーボード」かを動的に学習！
   - 自席で外付けUSキーボードを打鍵するとUS変換エミュレーションが自動適用され、ノートPC本体のキーボードを打鍵すると一切変換せずスルーされます。
