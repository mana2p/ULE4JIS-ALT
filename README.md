...
2. **キーボードデバイス自動識別 (1文字目学習・2文字目切り替え)** 🚀 *NEW*
   - 入力された最初の1文字目で「ノートPC本体のJISキーボード」か「外付けUSキーボード」かを取得します。
   - 外付けUSキーボードを打鍵するとUS変換エミュレーションが自動適用され、ノートPC本体のキーボードを打鍵すると一切変換せずスルーされます。

3. **左右 Alt 空打ちによるスマート IME 切り替え (macOS風)**
   - **左 Alt の空打ち**: 無変換キー (`VK_NONCONVERT`) を送信し、英語入力 (IME OFF) へ変換。
   - **右 Alt の空打ち**: ひらがなキー (`VK_HIRAGANA`) を送信し、日本語入力 (IME ON) へ変換。
